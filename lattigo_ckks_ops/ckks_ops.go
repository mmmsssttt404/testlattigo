...
//
// Context dir files (created by create_context(dir)):
//
//	parm.lattigo  (gob ParamDesc)
//	pub.lattigo   (PK MarshalBinary)
//	sec.lattigo   (SK MarshalBinary)
//	relin.lattigo (RLK MarshalBinary)
//	gal.lattigo   (pow2 galois key pack)
//
// HEVM ops:
//
//	op is 8 bytes on disk: uint16 opcode,dst,lhs,rhs (little-endian)
//
// NOTE:
//
//	This backend implements a "pseudo bootstrap" (decrypt->decode->encode->encrypt) for OP_BOOT.
//	It requires SK/PK (ROLE_FULL/ROLE_CLIENT). Server-only VM cannot do pseudo boot.
package main
...
// IMPORTANT: configBody MUST match your generator and your working hevm_reader.go:
// exactly 5x u64 = 40 bytes.
...
func saveBin(path string, b []byte) error { return os.WriteFile(path, b, 0o644) }
func loadBin(path string) ([]byte, error) { return os.ReadFile(path) }

// -------------------- SEAL-like params: N=15, L=14 -> maxLevel=13 --------------------
// This matches your printed: logN=15, slots=16384, maxLevel=13, logDefaultScale=40.
func sealLikeParamsLiteralN15() ckks.ParametersLiteral {
	...
	return ckks.ParametersLiteral{
		LogN:            N,
		LogQ:            logQ,
		LogP:            []int{60}, // needed for relin/rot keys
		LogDefaultScale: 40,
	}
}
...
func (v *VM) initParams(name string, lit ckks.ParametersLiteral) error {
	...
	// Use 53-bit encoder like float64 mantissa
	v.encoder = ckks.NewEncoder(v.params, 53)
	return nil
}
...
func createContextDir(dir string) error {
	...
	logSlots := int(math.Log2(float64(params.MaxSlots()))) // 14
...
func (v *VM) loadClientOnly(dir string) error {
	if err := v.loadParm(dir); err != nil {
		return err
	}
	// pk
...
func (v *VM) loadServerOnly(dir string) error {
	if err := v.loadParm(dir); err != nil {
		return err
	}
	// rlk
...
// -------------------- HEVM loader (MUST MATCH hevm_reader.go) --------------------
...
func (v *VM) loadHEVM(path string) error {
	...
	if err := binary.Read(f, binary.LittleEndian, &cfg); err != nil {
		return fmt.Errorf("read configBody(40B): %w", err)
	}
	v.cfg = cfg
	...
	if err := readU64Arr(v.argScale); err != nil {
		return fmt.Errorf("read argScale: %w", err)
	}
	if err := readU64Arr(v.argLevel); err != nil {
		return fmt.Errorf("read argLevel: %w", err)
	}
	if err := readU64Arr(v.resScale); err != nil {
		return fmt.Errorf("read resScale: %w", err)
	}
	if err := readU64Arr(v.resLevel); err != nil {
		return fmt.Errorf("read resLevel: %w", err)
	}
	if err := readU64Arr(v.resDst); err != nil {
		return fmt.Errorf("read resDst: %w", err)
	}

	numOps := int(cfg.NumOperations)
	if numOps < 0 || numOps > 50_000_000 {
		return fmt.Errorf("num_operations invalid: %d", numOps)
	}
	v.ops = make([]hevmOp, numOps)

	// Read ops as raw 8 bytes each, decode 4x u16, EXACTLY like hevm_reader.go.
	for i := 0; i < numOps; i++ {
		var raw [8]byte
		...
		v.ops[i] = hevmOp{
			...
			Lhs:    binary.LittleEndian.Uint16(raw[4:6]),
			Rhs:    binary.LittleEndian.Uint16(raw[6:8]),
		}
	}
...
func run(h C.uintptr_t) {
	...
	for idx, op := range v.ops {
		if v.debug {
			fmt.Printf("[OP] #%d opcode=%d dst=%d lhs=%d rhs=%d\n", idx, op.Opcode, op.Dst, op.Lhs, op.Rhs)
		}
		...
		case OP_NEGATEC:
			...
			// official Mul supports scalar; negate == Mul(-1)
		...
		case OP_MODSWC:
			...
			// DropLevel is official (no rescale)
			*out = *in.CopyNew()
			if down > 0 {
		...
		case OP_UPSCALEC:
			// keep unimplemented for your SEAL-like pipeline unless you map it to ScaleUp/SetScale.
			continue
		...
		case OP_ADDCC:
			...
			// Official Add does: min level selection + scale handling internally
		...
		case OP_ADDCP:
			...
			// Official Add accepts rlwe.Operand; plaintext implements ElementInterface
		...
		case OP_MULCC:
			...
			// Use official MulRelin for ct*ct (degree 1 result)
		...
		case OP_MULCP:
			...
			// Official Mul accepts plaintext operand
